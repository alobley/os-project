...
static uint32 LocalApicIn(uint32 reg){
    return mmioreadl(localApicAddr + reg);
}

static void LocalApicOut(uint32 reg, uint32 data){
    mmiowritel(localApicAddr + reg, data);
}
