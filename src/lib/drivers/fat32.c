...
BIOS_parameter_block_t* ReadFat32Header(disk_info_t* disk){
    ...
    uint16* buffer = (uint16* )kmalloc(MBR_SECTOR_SIZE);

    ReadSectors(disk, lbaOffset, sectorCount, buffer);
    if(buffer == NULL){
        return NULL;
    }
...
void ParseRoot(fat_fs_t* fs, disk_info_t* disk){
    ...
    uint16* buffer = (uint16* )kmalloc(MBR_SECTOR_SIZE * fs->sectorsPerCluster);
...
void* LoadFile(fat_fs_t* fs, disk_info_t* disk, void* loadAddress, char* fileName){
    ...
    for(size_t i = 0; i < numEntries; i++){
        ...
        while(cluster >= 2 && cluster < 0xFFFFFF8 && bytesRemaining > 0){
            ...
            ReadSectors(disk, cluster, sectorCount, buffer);
            ...
            uint32 fatBuffer;
            ReadSectors(disk, fatSector, 1, (uint16*)&fatBuffer);
